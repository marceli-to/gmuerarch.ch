...
<div>
  ...
  <div v-if="isFetched" class="is-loaded">
    <page-header>
      <h1>Team</h1>
      <router-link :to="{ name: 'team-member-create'}" class="btn-add has-icon">
...
export default {
  ...
  methods: {
    ...
    order() {
      let members = this.data.map(function(member, idx) {
        member.order = idx;
        return member;
      });
      ...
      this.debounce = setTimeout(function() {
        this.debounce = false 
        this.axios.post(`${this.routes.order}`, {members: members}).then((response) => {
